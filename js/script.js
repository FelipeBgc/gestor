...
function populateProductOptions() {
    ...
    groupedProducts.forEach((group, index) => {
        ...
        const priceText = group.batches.length === 1 ? ` — ${formatMoney(group.batches[0]?.sellingPrice || 0)}` : '';
        option.textContent = `${group.product}${group.size ? ` (${group.size})` : ''} — Qtd: ${group.quantity}${priceText}`;
...
function populateBatchOptions() {
    ...
    if (group.batches.length > 1) {
        ...
        group.batches.forEach((batch, batchIdx) => {
            ...
            option.textContent = `Lote ${batchIdx + 1} - Qtd: ${batch.quantity} - ${formatMoney(batch.sellingPrice || 0)} (${batch.purchaseLocation || 'sem local'})`;
